...
export const releaseSlice = createSlice({
  ...
  initialState,
  reducers: {
    setModules: (state, {payload: modules}: PayloadAction<ReleaseWithModuleVersions['modules']>) => {
    ...
    deleteModule: (state, {payload: moduleSlug}: PayloadAction<string>) => {
      state.modules = state.modules.filter(module => module.moduleSlug !== moduleSlug)
    },
  },
})
